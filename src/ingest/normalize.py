...
_RULES: list[tuple[str, list[str]]] = [
    ...
    # 법인세비용 — 마법공식 EBIT(=당기순이익+법인세비용+이자비용) 계산용. '법인세비용차감전
    # 순이익'은 '차감전'이 _EXCLUDE로 사전 차단되므로 여기 오매칭되지 않는다.
    ("tax_expense", ["법인세비용"]),
    ...
    ("depreciation", ["감가상각비"]),
    # 현금및현금성자산(재무상태표 스냅샷) — 마법공식 여유자금(excess cash) 계산용.
    # 현금흐름표의 '기초/기말 현금및현금성자산'은 '기초'/'기말'이 _EXCLUDE로 차단되어
    # 여기 오매칭되지 않는다(재무상태표 시점 잔액 하나만 잡는다).
    ("cash", ["현금및현금성자산"]),
...
# '기초'/'기말'은 현금흐름표의 '기초/기말 현금및현금성자산'(흐름표 기간 항목)이 재무상태표
# cash('현금및현금성자산' 시점 잔액) 패턴에 부분문자열로 오매칭되는 것을 차단한다.
# '환율변동'/'증가'/'감소'/'증감'/'환산효과'도 같은 이유 — 현금흐름표의 환율변동효과·순증가(감소)·
# 증감 항목이 '현금및현금성자산'을 부분문자열로 포함해 cash로 오매칭되는 것을 막는다(실제
# data/market.db raw_reports에서 확인된 오염). 우리가 매핑하는 계정명 중 이 수식어를 포함하는
# 것은 없어 정상 매핑에는 영향이 없다(오매칭만 차단).
_EXCLUDE_KEYWORDS = ["차감전", "차감후", "계속영업", "중단영업", "주당", "자본과부채", "매출채권",
                     "기타영업", "기타수익", "기타의영업", "투자영업", "비지배", "기초", "기말",
                     "환율변동", "증가", "감소", "증감", "환산효과"]
...
_DART_ID_MAP = {
    ...
    # 마법공식(EY/ROC)용 — 법인세비용 / 재무상태표 현금및현금성자산(시점 잔액).
    # 현금흐름표 기초/기말 현금(dart_...AtBeginning/EndOfPeriodCf)은 여기 없어 cash로 안 잡힌다.
